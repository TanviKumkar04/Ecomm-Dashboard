...
import { Menu, X, Home, LogIn, LogOut, Package, UserPlus } from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <div className="flex">
      <div className={`${open ? "w-64" : "w-16"} bg-gray-900 h-screen p-5 pt-8 duration-300 fixed left-0 top-0`}>
        ...
        <ul className="mt-8 space-y-4">
          ...
          {!loggedIn ? (
            <>
              <li>
                <Link href="/login" className="flex items-center gap-4 text-gray-300 p-2 hover:bg-gray-700 rounded-md">
                  <LogIn size={20} /> {open && "Login"}
                </Link>
              </li>

              <li>
                <Link href="/register" className="flex items-center gap-4 text-gray-300 p-2 hover:bg-gray-700 rounded-md">
